import { ArrowRight, BookOpen, CheckCircle2, Database, FileText, Gamepad2, MessageCircle, Save, Search, Sparkles, Target, Trophy } from 'lucide-react';
...
export function TablePlayDevice({ language, onNavigate }: { language: Language; onNavigate: (section: Section) => void }) {
  ...
  const saveProgress = () => {
    const record = saveSessionProgressRecord({
      ...
      focusTitle: selectedGoal.title,
      conversationCard: selectedPrompt,
      usefulPhrase: usefulPhrase.trim() || selectedGoal.phrases[0],
      whatHappened: whatHappened.trim() || `Used "${selectedPrompt}" during table play.`,
      nextTime: nextTime.trim() || 'Try one more sentence with the same focus next time.',
    });
    setSaveMessage(`${record.gameTitle} table play device saved to Progress.`);
  };
              ...
              <div className="p-6">
                    ...
                    {conversationPrompts.map((prompt) => {
                      ...
                      return (
                        <button
                          ...
                          className={`rounded-xl border p-4 text-left text-sm leading-6 transition hover:bg-white ${
                ...
                <section className="mt-5 rounded-2xl border border-[#ead4fa] bg-[#fdf8ff] p-5">
                  ...
                  <div className="mt-3 grid gap-2 sm:grid-cols-3">
                    {selectedGoal.phrases.map((phrase) => (
                      <button
                        key={phrase}
                ...
                <section className="mt-5 rounded-2xl border border-[#efc779] bg-white p-5">
                  ...
                  <div className="mt-4 grid gap-4">
                    <label className="text-xs font-bold uppercase tracking-wide text-[#7a7065]">
                      ...
                      <input value={usefulPhrase} onChange={(event) => setUsefulPhrase(event.target.value)} className="mt-2 w-full rounded-xl border border-[#efc779] px-4 py-3 text-sm font-normal normal-case outline-none focus:border-[#d87522]" placeholder={selectedGoal.phrases[0]} />
                    </label>
                    <label className="text-xs font-bold uppercase tracking-wide text-[#7a7065]">
